...
import { Menu, X, MapPin, Book, Landmark, Palette, User, LogOut, CheckCircle, Heart, LogIn, Flag, BookOpen, Calendar, ShieldAlert, Wallet } from "lucide-react";
import { cn } from "@/lib/utils";
import { useAuth } from "@/contexts/AuthContext";
import { Avatar, AvatarFallback } from "@/components/ui/avatar";
import CurrencyConverter from "../../features/CurrencyConverter";
import LanguageHub from "../../features/LanguageHub";
import PackingAssistant from "../../features/PackingAssistant";
import EmergencyHelpline from "../../features/EmergencyHelpline";
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="gap-2">
                <span className="hidden xl:inline">Travel Tools</span>
                <span className="xl:hidden">Tools</span>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => setOpenTool("packing")}>
                <Book className="mr-2 h-4 w-4" /> Packing Assistant
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setOpenTool("language")}>
                <BookOpen className="mr-2 h-4 w-4" /> Language Hub
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setOpenTool("currency")}>
                <Landmark className="mr-2 h-4 w-4" /> Currency Converter
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => setOpenTool("expense")}>
                <Wallet className="mr-2 h-4 w-4" /> Expense Tracker
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setOpenTool("emergency")} className="text-red-600 focus:text-red-700 focus:bg-red-50">
                <ShieldAlert className="mr-2 h-4 w-4" /> Emergency Helpline
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>

          {/* Controlled Dialogs */}
          <PackingAssistant open={openTool === "packing"} onOpenChange={(open) => !open && setOpenTool(null)} />
          <LanguageHub open={openTool === "language"} onOpenChange={(open) => !open && setOpenTool(null)} />
          <CurrencyConverter open={openTool === "currency"} onOpenChange={(open) => !open && setOpenTool(null)} />
          <ExpenseTracker open={openTool === "expense"} onOpenChange={(open) => !open && setOpenTool(null)} />
          <EmergencyHelpline open={openTool === "emergency"} onOpenChange={(open) => !open && setOpenTool(null)} />
          {isAuthenticated ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
